\section{基于超级行程的下界模型 $\model{\discretization, \itinSetSuper}$}
\label{sec:lbm}

为解决完整时空网络的可处理性问题，我们在聚合时空网络和超级行程空间上建立松弛模型。该模型的核心思想是：在一个较粗的工作离散化$\discretization$上构造时空网络，并将原始行程按路径签名合并为超级行程，从而大幅减小问题规模，同时通过超级行程吸引力上界的构造保证有效的理论下界。该模型忽略了某些严格的时间可行性要求，这些要求在上界修复模型和聚合表示细化阶段中得到解决。
...
\paragraph{拓扑自适应变量压缩。}
异构步长自然适应真实世界服务网络的枢纽辐射结构。接收许多短途服务的枢纽节点获得较小的$\initStep_i$，因此获得保持时间精度的更细网格。主要由长途航段服务的辐射节点获得较大的$\initStep_i$和相应更粗的网格。这种自适应行为在不牺牲网络物理有效性的情况下保持初始时间点总数——从而保持决策变量数量——紧凑，避免了当均匀细密网格应用于大规模实例时出现的内存溢出故障。
