为改善选择约束中的数值病态，本文在所有模型求解前对吸引力参数进行市场级缩放预处理。对于每个市场$m \in \marketSet$和顾客类型$p \in \psgTypeSet$，识别最小非零绝对吸引力值：
\begin{equation*}
...
\end{equation*}
然后应用缩放因子$\attrScale_{mp} = 1.0 / \attrMin_{mp}$均匀缩放该市场-顾客组合内的所有吸引力参数：
\begin{equation*}
\begin{aligned}
...
由于选择约束\eqref{constr:main.choice.ratio}和\eqref{constr:main.choice.limit}在每个市场-顾客组合内对吸引力参数是齐次的，将所有吸引力值乘以正常数$\attrScale_{mp}$严格保持可行域和最优解。为验证这一点，观察约束\eqref{constr:main.choice.ratio}变为：
\begin{equation*}
