...
\begin{tikzpicture}
  ...
  \begin{scope}[shift={(-3,0)}, anchor=north west]
    \node[font=\bfseries] (title1) at (0, 2.5) {原始行程候选集};
    \begin{scope}
      ...
      \node[labelNode, align=left, anchor=north west] (labelNode) at (0, 2) {
        ...
        \begin{tabular}{lccc}
          ...
          从 $j$ 出发 & 6:00 & 6:30 & 7:00 \\ [6pt]
          路径 & \multicolumn{3}{c}{$i\to j \to k$} \\
          \bottomrule
        \end{tabular}
        \\ [6pt]
    ...
    \begin{scope}[shift={(7.3,0)}, anchor=north west]
      \draw[timeaxis] (0,0) -- (6,0) node[right, scale=1] (jlabel) {节点 $j$};
      \draw[timeaxis, gray] (0,1) -- (6,1) node[right, black, scale=1] (klabel) {节点 $k$};
      ...
      % Conflict markers
      \draw[conflict] (1.3, 0.4) -- ++(1.8, 0) node[midway, below, red, scale=0.6] {冲突} ++(0,0.4) node[midway, above, red, scale=0.4] {($<45 mins$)};
      \draw[conflict] (3.1, 0.7) -- ++(1.8, 0) node[midway, below, red, scale=0.6] {冲突} ++(0,0.4) node[midway, above, red, scale=0.4] {($<45 mins$)};
    ...
    % lower background — drawn later with correct right edge
  ...
  \begin{scope}[shift={(-3,-2.0)}, anchor=south west]
    \node[font=\bfseries] (title2) at (0, 0) {有效组合（子集）};
  ...
  \begin{pgfonlayer}{background}
    ...
    % Also extend top background to attrDist2.east
    \path let \p1 = (attrDist2.east),
              \p2 = (title1.north west)
    in
      coordinate (bgEnd1Right) at (\x1, 0);
    \fill[gray!5, rounded corners=2mm] (title1.north west) rectangle (bgEnd1Right |- bgEnd);
  \end{pgfonlayer}
  ...
  \node[scale=0.6, red, align=center] at ($(downArrowStart)$) {时刻表 \\ 约束};
